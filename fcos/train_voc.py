...
DATASETPATH = '/mnt/data/ai4phys/weigengchen/datasets/VOC2012/VOC2012/VOCdevkit/VOC2012'

wandb.init(project="FCOS", config=opt)

transform = Transforms()
train_dataset = VOCDataset(root_dir=DATASETPATH,resize_size=[800,1333],
...
for epoch in range(EPOCHS):
    for epoch_step, data in enumerate(train_loader):
        ...
        if epoch_step // 20 == 0:
            wandb.log({
                ...
                "epoch": epoch,
                "steps": epoch_step+1,
                "cls_loss": float(losses[0].mean()),
                "cnt_loss": float(losses[1].mean()),
                "reg_loss": float(losses[2].mean()),
                "cost_time": cost_time,
                "lr": lr,
    ...
    checkpoint_name = "./checkpoint_4gpu_train/model_{}.pth".format(epoch + 1)
